...
double calcularMedia(List<Map> listaAlunos) {
  double somaNotasAlunos = listaAlunos.map((aluno) => aluno['notaAluno']).reduce((a, b) => a + b); //mapeando elementos com o Map e os juntando em um único valor com Reduce
...
void imprimirResultadosTotal(
  ...
  for (var aluno in filtrarAlunosAprovados) {//vai percorrer cada elemento da lista e mostrar os alunos aprovados
    print('Nome: ${aluno['nomeAluno']}, Nota: ${aluno['notaAluno']}'); //imprime o nome do aluno e sua respectiva nota | media
  }
}
...
void main() {
  ...
  while (true) {
    stdout.write('Digite o nome do aluno ou "sair": '); //informar nomes ou sair do loop
    String nomeAluno = stdin.readLineSync()!; //lê as informações inseridas, assim como na 'notaAluno'
    if (nomeAluno == 'sair') { //se o usuário digitar "sair", o loop será interrompido
      break;
    }
  ...
  List<Map> alunosAprovados = filtrarAlunosAprovados(listaAlunos, mediaAlunos); //lista os alunos aprovados
  imprimirResultadosTotal(mediaAlunos, alunosAprovados); //mostrará o resultado final com a média da turma e aprovados
}
